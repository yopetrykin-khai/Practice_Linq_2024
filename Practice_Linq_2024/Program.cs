...
namespace Practice_Linq_2024
{
    internal class Program
    {
        ...
        static void Query9(List<FootballGame> games)
        {
            ...
            var selectedGames = games.Where(t => t.Tournament == "UEFA Nations League" && t.Date.Year == 2023)
                .Select(g => new { MatchYear = g.Date.Year, Game = $"{g.Home_team}-{g.Away_team}", Result = g.Home_score > g.Away_score ? "Win" : g.Home_score < g.Away_score ? "Loss" : "Draw", });   // Корегуємо запит !!!
